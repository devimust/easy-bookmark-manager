	...
	var userEditController = function ($scope, $location, $http, $interval, userService) {
		...
		userService
			...
			.then(function (response) {
				...
				$scope.profile.username = response.data.user.username;
				$scope.profile.admin = response.data.user.admin;
			});
		...
		$scope.updateUser = function () {
			...
			if ($scope.profile.password1.length > 0) {
				if ($scope.profile.password1.length < 6) {
				...
				if ($scope.profile.password1 != $scope.profile.password2) {
					$scope.errorMessage = 'Your new and old passwords must match';
					return;
				}
